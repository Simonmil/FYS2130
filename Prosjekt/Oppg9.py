...
for t in range(4000):
	for i in range(1,int(N)-1):
		y_pluss[i] = (dt**2*(-(k[i-1] + k[i])*y_0[i]\
		 + k[i-1]*y_0[i-1] + k[i]*y_0[i+1]))/m[i] + 2*y_0[i] - y_minus[i]
...
plt.plot(all_pos[1000], label=r'$t_{step} = 500$')
plt.plot(all_pos[1750], label=r'$t_{step} = 1750$')
